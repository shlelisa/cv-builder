...
const LetterGenerator: React.FC = () => {
  ...
  const parseProfile = (text: string): Partial<UserProfile> => {
    ...
    ['react', 'node', 'mysql', 'javascript', 'typescript', 'python', 'java', 'html', 'css', 'postgresql', 'mongodb', 'sql', 'git', 'aws', 'excel', 'powerpoint'].forEach((skill) => {
  ...
  const handleGenerate = () => {
    ...
    const letter =
      letterType === 'application'
        ? aiService.generateApplicationLetter(profile, company, position, requirements.split('\n').filter((r) => r.trim()), language)
    ...
    <div className="max-w-6xl mx-auto p-6">
      <div className="mb-8">
        ...
        <p className="text-gray-600 dark:text-zinc-400">
          Generate professional application letters and cover letters tailored to specific positions.
        </p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        <div className="bg-white dark:bg-zinc-900 rounded-lg border border-gray-200 dark:border-zinc-800 p-6 space-y-4">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-zinc-100">Letter Details</h3>
          ...
          <Button onClick={handleGenerate} className="w-full">
            Generate {letterType === 'application' ? 'Application' : 'Cover'} Letter
          </Button>
        </div>
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-lg border border-gray-200 dark:border-zinc-800 p-6">
          ...
          {generatedLetter ? (
            ...
            <div className="text-center py-12 text-gray-400 dark:text-zinc-500 border-2 border-dashed border-gray-200 dark:border-zinc-700 rounded-lg">
              Fill in the details and click &quot;Generate&quot; to create your letter.
            </div>
          )}
        </div>
      </div>
